...
export const register = async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { uuid: newUser.uuid, email: newUser.email, role: userRole.name },
...
export const login = async (req, res) => {
  try {
    ...
    // Buat JWT token
    const token = jwt.sign(
      { uuid: user.uuid, email: user.email, role: user.role.name },
      process.env.JWT_SECRET,
      { expiresIn: "7d" }
    );
...
export const logout = async (req, res) => {
  try {
    // Serverless: cukup beri response, client hapus token sendiri
